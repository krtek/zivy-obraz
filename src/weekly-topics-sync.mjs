...
const geminiApiKey =
  resolveValue("gemini-api-key", 3) ?? process.env.GOOGLEAI_API_KEY;
...
async function fetchWeeklyTopics() {
  ...
  const rawSubjects =
    subjectsResponse.data?.Subjects ?? subjectsResponse.data?.subjects ?? [];
  ...
  const days =
    timetableResponse.data?.Days ?? timetableResponse.data?.days ?? [];
  ...
  for (const day of days) {
    ...
    const dateStr =
      typeof raw === "string" ? raw.slice(0, 10) : toIsoDate(new Date(raw));
    ...
    const atoms =
      day?.Atoms ?? day?.Lessons ?? day?.atoms ?? day?.lessons ?? [];
...
${JSON.stringify(result, null, 2)}`;
    ...
    const now = new Date();
    const weekLabel = new Intl.DateTimeFormat("cs-CZ", {
      day: "numeric",
      month: "long",
      year: "numeric",
    }).format(now);
